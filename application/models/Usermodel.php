...
class Usermodel extends commonmodel {
	...
	public function insertUserBasic( $param ) {
		...
		if ( $statement->execute() ) {
			...
			in_array( $param['acc_type'], array ( 3 ) ) ?
				$this->insertUserAccountType( array ( 'user_id'     => $response['user_id'],
				                                      'acc_type_id' => $param['acc_type'],
				                                      'status'      => 'active'
				) ) : '';
		} else {
	...
	public function getextensionalreadydata( $member_id ) {
		$query  = "SELECT *
                        FROM tbl_karmora_kash_account
                        WHERE fk_user_id = $member_id AND kash_description = 'kash back toolbar' LIMIT 1";
...
//              exit;
	...
	public function getOrderDetailById( $order_no ) {
		$query   = "Select view_my_order_detail.*,tbl_users.user_first_name,tbl_users.user_last_name,tbl_users.user_username ,tbl_users.user_email,
                    tbl_users.user_phone_no
                    from view_my_order_detail ,tbl_users
                    where  view_my_order_detail.pk_order_id = $order_no and tbl_users.pk_user_id = view_my_order_detail.fk_user_id";
		$queryRS = $this->db->query( $query );
	...
	public function getOrderProduct( $order_no ) {
		$query = "SELECT tbl_order_line.*,tbl_product.product_title,tbl_product.product_price as MSRP FROM `tbl_order_line` , tbl_product  WHERE tbl_order_line.`fk_order_id` = $order_no and tbl_order_line.fk_product_id = tbl_product.pk_product_id";
		$queryRS = $this->db->query( $query );
	...
	public function getAllOrder() {
		$sql     = "select view_my_orders.*,tbl_users.user_first_name,
                tbl_users.user_last_name
                from view_my_orders,tbl_users
                where tbl_users.pk_user_id = view_my_orders.`fk_user_id`";
		$queryRS = $this->db->query( $sql );
		if ( $queryRS->num_rows() > 0 ) {
			return $queryRS->result_array();
		} else {
	...
	public function getAllW9forms() {
		
		$sql     = "select tbl_user_w9_form.*,tbl_users.user_first_name,
                tbl_users.user_last_name
                from tbl_user_w9_form,tbl_users
                where tbl_users.pk_user_id = tbl_user_w9_form.`fk_user_id`";
		$queryRS = $this->db->query( $sql );
		if ( $queryRS->num_rows() > 0 ) {
			return $queryRS->result_array();
		} else {
	...
	public function getprintAllOrder( $type ) {
		$and = '';
		if ( $type != null ) {
			$and = 'AND email_create_date = ' . $type;
		}
		$sql     = "select view_my_orders.*,tbl_users.user_first_name,
                tbl_users.user_last_name,tbl_users.user_username,tbl_users.user_email,
                tbl_users.user_phone_no
                from view_my_orders,tbl_users
                where tbl_users.pk_user_id = view_my_orders.`fk_user_id`
                and view_my_orders.status != 'declined' and view_my_orders.status != 'return'
                and view_my_orders.status != 'canceled'
                $and ";
		$queryRS = $this->db->query( $sql );
		if ( $queryRS->num_rows() > 0 ) {
			return $queryRS->result_array();
	...
	public function getprintAllOrderfilter( $filter ) {
		$sql     = "select view_my_orders.*,tbl_users.user_first_name,
                tbl_users.user_last_name,tbl_users.user_username,tbl_users.user_email,
                tbl_users.user_phone_no
                from view_my_orders,tbl_users
